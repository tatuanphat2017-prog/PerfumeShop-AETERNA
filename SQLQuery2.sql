...
UPDATE dbo.Products SET Notes = N'Bergamot, Tiêu Sichuan' WHERE Name = N'Dior Sauvage EDT' AND (Notes IS NULL OR Notes = N'');
UPDATE dbo.Products SET Notes = N'Bạc hà, Vani' WHERE Name = N'Versace Eros EDT' AND (Notes IS NULL OR Notes = N'');
UPDATE dbo.Products SET Notes = N'Muối biển, Cam chanh' WHERE Name = N'Nautica Voyage' AND (Notes IS NULL OR Notes = N'');
